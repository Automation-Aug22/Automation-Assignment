...
def create_newOrder(headers):
    ...
    if response.status_code == 201:
        ...
        json_order_id = response_data.get("orderId")
        if json_order_id:
            with open("order_id.json", "w") as json_file:
                json.dump({"orderId": json_order_id}, json_file)
            print(f"Order ID {json_order_id} saved to 'order_id.json'")
        else:
...
def get_order_id_from_json():
    try:
        with open("order_id.json", "r") as json_file:
            data = json.load(json_file)
            return data.get("orderId")
    except FileNotFoundError:
        print("The 'order_id.json' file does not exist.")
        return None
...
if __name__ == "__main__":
    ...
    #fetching order ID from Json File.
    order_id_json = get_order_id_from_json()
    if order_id_json:
        print(f"Fetched Order ID from Json File: {order_id_json}")
        OrderIDJson = getSingleOrder(orderId)
        print(f'Fetching orderID from Json file and printing the details : {order_id_json}:- {OrderIDJson} \n')
        logging.info(f'Fetching orderID from Json file and printing the details : {order_id_json}:- {json.dumps(response4, indent=4)}')
    ...
    #deleting the Order
    order_Id = "Xs46DuQVOGcc54_h9cdXV"
